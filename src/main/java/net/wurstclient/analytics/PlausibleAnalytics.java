...
import net.wurstclient.forge.WurstForgeMod;
...
public final class PlausibleAnalytics
{
	...
	public PlausibleAnalytics(Path configFile)
	{
		...
		sessionProp("fabric_api_version",
			getVersion("fabric_api", "fabric-api"));
		sessionProp("fabric_loader_version",
			getVersion("connectormod", "connector"));
		sessionProp("modmenu_version", getVersion("modmenu"));
		sessionProp("sodium_version", getVersion("sodium"));
		sessionProp("sinytra_connector_version",
			getVersion("connectormod", "connector"));
		
		Thread thread = new Thread(this::runBackgroundLoop, "Plausible");
	...
	{
		return WurstForgeMod.getVersion(modId);
	}
	
	private String getVersion(String... modIds)
	{
	...
	{
		return WurstForgeMod.isDevelopmentEnvironment()
	...
	private String createRequestBody(PlausibleEvent event)
	{
		...
		if(event.props() != null && event.props().size() > 0)
